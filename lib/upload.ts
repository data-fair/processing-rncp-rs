 ...
 * concepts, separators, enums) and the dataset description, so the processing fully drives the
 * dataset metadata. The dataset stays a file dataset; its id and type are preserved on update.
 */
export const upload = async (context: RncpRsProcessingContext, csvPath: string): Promise<void> => {
